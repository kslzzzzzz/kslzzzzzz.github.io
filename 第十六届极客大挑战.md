...
# Use Fermat's factorization method to find p and q
# Since q = next_prime(p + 2^400), they are close to each other
# We can approximate p as the square root of hint
p_approx = gmpy2.isqrt(hint)
print(f"p_approx = {p_approx}")

# Search for p and q in a smaller range
found = False
...
if not found:
    ...
    # Try a different approach - use the fact that q = next_prime(p + 2^400)
    # So p and q are close to each other relative to their size
    # We can use Fermat's factorization method more effectively
    a = gmpy2.isqrt(hint)
    b2 = a*a - hint
...
# Since e=8, we can use the fact that p, q, r are all ≡ 3 mod 4
# This makes it easier to compute roots modulo these primes
...
def mod_root_8(c, p):
    ...
    # Compute square root three times
    root2 = mod_sqrt(c, p)
    root4 = mod_sqrt(root2, p)
    root8 = mod_sqrt(root4, p)
    return root8

# Compute possible roots modulo p, q, r
...
# Recover p and q using low-bit recovery
...
#### exp https://chat.deepseek.com/share/0kc6plxk8pv99wgg17                                               
